...
def starter(current: CurrentState, gen_speed: float = 0.0,
    ...
    print('This is the active theme:', current.active_theme)
...
def change_maze(current: CurrentState) -> CurrentState:
    ...
    if current.entry in logo_cells:
        print(f"{red}\nError: Entry {current.entry} hit '42' logo during " +
              f"resize to {current.w}x{current.h}{reset}")
        os._exit(0)

    if current.exit in logo_cells:
        print(f"{red}\nError: EXIT {current.exit} hit '42' logo during " +
              "resize to {current.w}x{current.h}{reset}")
        os._exit(0)
...
def load_themes(maze: MazeGenerator,
                ...
                win: Window,
                maze_themes: dict[str, list[int]] =
                maze_themes) -> dict[str, dict[str, Any]]:
...
def put_logo(current: CurrentState) -> Optional[tuple[int, int, c_void_p]]:
    render: Renderer = current.render
    win: Window = current.win
    theme_names: list[str] = current.theme_names
    theme_index: int = current.theme_index
...
def show_img(current: CurrentState, overlay: bool = False) -> None:
    if not current.algo_gen or not current.starter_frame:
        return
    ...
    if current.algo_anim:
        try:
            ...
            import __main__
            if hasattr(__main__, 'save_maze_now') and hasattr(__main__,
                                                              'output_file'):
        ...
        if not active_theme.get('path'):
            path: list = []
            bg = current.win.create_copy(active_theme['bg'])
        ...
        frames: list | Any = active_theme.get('path') if isinstance(
            active_theme.get('path'), list) else []
